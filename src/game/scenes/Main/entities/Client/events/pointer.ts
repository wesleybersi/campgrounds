...
export function pointerEvents(this: Client) {
  const pointerMove = () => {
    this.scene.input.on("pointermove", (pointer: Phaser.Input.Pointer) => {
      ...
      // }
      // const object = this.scene.grid.objectMatrix[row][col];
      // if (object && object instanceof Tree) {
      //   object.setAlpha(0.5);
      // }
    });
  };
  ...
  const pointerDown = () => {
    this.scene.input.on("pointerdown", (pointer: Phaser.Input.Pointer) => {
      ...
      if (pointer.rightButtonDown()) {
        ...
        // this.placeMode = "";
        if (
          this.selected instanceof Builder ||
          this.selected instanceof Forester
        ) {
          this.selected?.goto(col * CELL_SIZE, row * CELL_SIZE);
        }
      }
    });
  };
        ...
        if (this.selection) {
          ...
          for (const { col, row } of cells) {
            if (pointer.leftButtonReleased()) {
              ...
              if (objInPlace) {
                if (objInPlace instanceof Wall || objInPlace instanceof Hedge) {
                  new Task(
                    this.scene,
                    "builder",
                    col,
